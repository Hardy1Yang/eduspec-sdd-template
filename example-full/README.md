...
> 完整步驟見 [`操作走查.md`](操作走查.md)（Step 0→8，標明 👤 人工 vs 🤖 agent 自動）。
...
|------------------|------------------------------|----------------------------------|
| 一個混合資料夾：3 份講義 .tex ＋ 課綱/作業/考古題/案例/專案規範 PDF | `用「teach-context」這個 skill，講義在：rawcourse/。` | [`agent/course-context/`](agent/course-context/)（w1–w3 精華＋index 主題地圖）、[`agent/supplementary/`](agent/supplementary/)（五分型檔，無解答） |
| （上面的蒸餾結果） | `用「teach-agent」這個 skill，課名：…，授課教師：…，對象：…。` | 整個 [`agent/`](agent/)：[`AGENT.md`](agent/AGENT.md)（大腦）、[`knowledge-graph.md`](agent/knowledge-graph.md)、references/、`.claude/`＋`.codex/` 雙轉接頭、README、使用手冊 |
| （同上） | `用「teach-prereq」這個 skill，課名：…，對象：…。` | [`agent/prerequisites.md`](agent/prerequisites.md)（老師端清單〔課綱要求〕標示＋學生自我檢測） |
...
4. **看 SDD 長什麼樣**：讀 `sdd-archive/2026-07-10-w2-ols-sim/` 內的 proposal/spec/tasks——互動教材就是照這份規格做出來的。

## 品質保證（實測驗證過）

依賴檢查 exit 0、8/8 檔分類正確、零解答外洩、agent 四行為探針 5/5（概念完整教／作業只給提示／課務標「以公告為準」／超綱不編造）、sim 的 OLS 數學經獨立手算驗證、動畫逐幀複驗。細節見 [`操作走查.md`](操作走查.md) 的驗證彙總表。
