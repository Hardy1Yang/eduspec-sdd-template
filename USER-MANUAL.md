...
- **發布**：見助教的 `INSTRUCTOR-MANUAL.md` §12——**方式 A（推薦）zip 分享**（打包排除 `source-materials/`，學生免 git）；方式 B（選用）GitHub repo（之後更新方便；Mac 首次 git 會裝 Xcode CLT）。著作權與個資檢查在 §13。
...
生成物都是純 markdown，**agent 每次回答都會重讀檔案——你改了、它就變了**，不用重新生成、不用重開。

1. **調說話風格**：改 `<課程代碼>-ta/AGENT.md` 第一節（角色與語言）＋ `references/teaching-style.md`（教學口吻與慣用例）。例：把「耐心、精確」改成「幽默、愛用比喻」，`/ta` 下一題就變聲。
2. **加自己的規則**：加在 `AGENT.md` 對應節（作答規則、協助邊界…）。**一次只加一條、改完立刻用一題實測**——一次加一堆會不知道是哪條造成怪行為。
3. **加一個新 skill**（例：想支援影片教材、想加一種新教材類型）：到 `skills/` 複製一個**最接近你需求的** `SKILL.md` 資料夾，改觸發句、步驟與輸出路徑即可——skill 就是一份「做法說明書」，AI 讀得懂就能執行。
4. **直接改生成的內容**：`lecture-notes/`、`knowledge-graph.md`、`syllabus.md`…都可以手動編輯或叫 AI 改，存檔即生效。改完建議跑一次 `ACCEPTANCE-CHECKLIST.md` 相關探針。
...
| 學生用英文問可以嗎 | 可以——直接用英文問，助教會自動以英文回答（實測），不用改任何設定 |
