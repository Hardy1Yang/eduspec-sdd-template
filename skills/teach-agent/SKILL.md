...
## 6. 產 README（**學生優先**——發布後學生落地第一眼就是它）

README 結構固定為「上半給學生、下半給老師」：

**上半【給學生】**（學生只讀這段就能用）：
1. **這是什麼**：本課的 24 小時 AI 助教，用老師的講義與說法回答。
2. **三步開始用**：① 裝 Claude Code 或 Codex（附官方連結）→ ② `git clone` 本 repo → ③ 用 coding agent 開資料夾發問。
3. **怎麼問**（兩種工具並列）：Claude Code 打 `/ta <問題>`；Codex 用 `Use $<課程代碼>-ta-agent to answer: <問題>`。
4. **預期行為**：觀念題完整教＋引講義小節；**作業/考題只給提示不給答案**（刻意設計）；課務據課綱答；超綱不編造。
5. **誠信與免責**：AI 會出錯，交出去的內容自己負責；依課程綠黃紅燈規範使用並誠實揭露。
