...
報告的「course-context 回寫」欄依 §12b 的結果填寫。報告結構：**本次 AI 做了什麼**（開頭第 0 節：生成規則一句話一條＋本次實際生成清單——讓沒讀過 skill 的老師知道要驗收什麼）／生成摘要／已生成檔案／略過的選用模組／**需要人工檢查的檔案**（每項含：生成檔・原始來源・具體位置・問題原因・AI 採取的處理）／未使用或無法分類的原始檔／原始教材衝突／可能缺少的附件。**沒有問題就明寫「無」。**
...
## 12b. 回寫套件工作區（course-context）

生成完成後、git 收尾之前，把蒸餾好的講義**回寫**到套件的教材工作區，讓老師接著在套件 clone 內生投影片／小考時不用再手動搬講義：

1. **定位來源套件**：從觸發本 skill 當下的工作目錄往上找，取**第一個**根目錄含 `course-context/` 的資料夾（即老師下載的 EduSpec 套件 clone）。
2. 找到了 → 把 `<課程代碼>-ta/lecture-notes/*.md`（**排除 `README.md`**）複製到該套件根的 `course-context/`，**同名覆蓋**。
3. 找不到（例如在任意資料夾觸發、或套件根沒有 `course-context/`）→ **安全略過**：不報錯、不中斷初始化。
4. 無論結果如何，在 `INITIALIZATION-REPORT.md` 的「course-context 回寫」欄記一行：回寫了幾個檔到哪裡；或略過的原因＋手動補救指引（「可自行把 `lecture-notes/*.md` 複製到套件根 `course-context/`」）。

> 材料 skill 本來就會讀「course-context/ 或助教資料夾內的 lecture-notes/」；回寫是讓兩邊一致的保險，回寫失敗不影響在助教資料夾內生成教材。
