...
到 `course-context/`（套件根的，或助教資料夾內的 `agent/course-context/`——以實際放講義的那份為準） 確認概念。想清楚這動畫**要讓學生看到什麼過程**（不是塞一堆東西）。

> **選用·定位**：若有 `supplementary/syllabus.md`（與 `course-context/` 同層），參考主題在課程中的定位，讓動畫扣緊課程重點。沒有就照常。
