...
>
> **閘門二（scene 規劃列出後再停）**：寫程式前，先把逐幕規劃**寫成檔案 `scene-plan.md`**（與規格檔同資料夾；每幕一節：要做什麼、對應規格哪一幕、大約幾秒），印出路徑並請使用者確認**沒有超出規格範圍**，回「**go**」才開始寫 manim 程式與 render。動畫 render 要跑很久，先停這兩次反而省時間與額度。
