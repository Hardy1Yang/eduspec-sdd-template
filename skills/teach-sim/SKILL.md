...
- 本 skill 的「迷你規格」＝ SDD 流程中的 `proposal.md`（Why／教學意圖／範圍／驗收準則的精簡版）：**寫完 proposal.md 就是老師核可點**——依第 2／3 步的**兩個閘門**停等（proposal 核可才寫 specs/tasks；tasks 確認才實作），不要一路自動做到底。
...
> **閘門一（寫完 proposal.md 就停）**：印出檔案**完整路徑**，告訴使用者「請打開看一遍，哪裡不對直接改字（純 Markdown，存檔就算數），也建議在驗收準則加一條你課程專屬的檢核；回『**繼續**』（或 OK）我再寫 specs 與 tasks」。**停下等回覆，不要先寫後面的東西。**使用者若直接改了檔案，以改後版本為準續寫。
...
specs/ 與 tasks.md 寫完後**再停一次**：印出兩檔路徑，請使用者只檢查一件事——**任務清單有沒有超出 proposal 寫的範圍**（例：只要一頁模擬器，卻列了要架伺服器就是超了；輸入輸出對不對、要不要共享紀錄、驗收清單能不能打勾也順手掃）。**回『go』才開始實作**；不對就回頭改規格再走一次。
