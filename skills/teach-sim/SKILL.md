...
收到「繼續」後寫三份：**`design.md`**（上半＝**敘事／互動順序**——學生先看到什麼、拉了桿發生什麼、洞見句何時出現，這半是給老師確認的；下半＝技術決策——引擎、資料來源、離線/共享，老師可以不懂）、`specs/`（行為細則）、`tasks.md`（可打勾步驟）。寫完**再停一次**：印出 design 與 tasks 的路徑，請使用者檢查兩件事——**design 上半的敘事順序合不合教學直覺**、**任務清單有沒有超出 proposal 寫的範圍**（例：只要一頁模擬器，卻列了要架伺服器就是超了；輸入輸出、要不要共享紀錄也順手掃）。**回『go』才開始實作**；不對就回頭改規格再走一次。
