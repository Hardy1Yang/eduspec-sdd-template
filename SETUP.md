...
> ⚠️ **關於 AI 用量（先有心理準備）**：蒸餾教材（`teach-context`／`teach-agent`）會**平行開多個 sub-agent**、生成教材也會用 AI，**都會消耗你 Claude／Codex 的用量**。**第一次先從一個單元或一小部分開始**，別一次倒整包課程；**免費／低額度方案可能中途把 session 額度用完、跑不完**——遇到就分批做。這不是裝哪個工具能解決的，而是使用節奏問題。
