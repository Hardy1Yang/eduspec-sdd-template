...
> 下面每一步都是「在 Claude Code 對話裡貼一句話」。把粗體換成你的課。每個 skill 都會**先給你一份迷你規格請你確認、再生成、並附一份 `使用手冊-<類型>.md`**。
...
> 想看生成的教材長什麼樣：**完整實測全套見 [`example-full/`](example-full/)**（真實課程：助教＋教材＋SDD 歸檔＋走查）；迷你格式示範見 [`example/`](example/)。
